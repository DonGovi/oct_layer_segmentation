...
class LayerSeg(object):
    ...
    def imgGrad(self, kernel_size=9):
        '''
    ...
    def bdGrow(self, start, area=5, tau=1):
        height, width = grad_img.shape
        x, y = start
        y_start = self.getField(y, area)
        x_start = np.array([x]*area)
        start_energy = self.funcPhi(self.grad_img[y_start, x_start], tau)
        loc = np.argmin(start_energy)
        y += loc - int(area/2)
        
        boundary = np.zeros(self.grad_img.shape[1], dtype=np.float32)
        boundary[x] = y
        ...
        while(forward_x > 0):
            forward_x -= 1
            y_coords = self.getField(forward_y, area)
            x_coords = np.array([forward_x]*area)
            grad = self.grad_img[y_coords, x_coords]
            energy = self.funcPhi(grad, tau)
        ...
        while(backward_x+1 < width):
            ...
            y_coords = self.getField(backward_y, area)
